...
for i in ne #loop each element
    ...
    # s = 1
        ...
        all_set = Vector{Vector{Float64}}(undef, prod(length.(all_fc′, all_fpe, all_set)))
        for fc′ in all_fc′
            for as in all_as
                for fpe in all_fpe
                    feasible = filter([:fc′, :as, :fpe] => (x1,x2,x3) -> 
                        x1==fc′ &&
                        x2==as &&
                        x4==fpe,
                        sub_catalog
                        )
                    if size(feasible,1) != 0
                        push!(all_set, [fc′, as, fpe])
                    end
                end
            end
        end
...
#to do, properly find the similar fc′, As and fpe 
